...
class SpellCheckEngine:
    ...
    def _load_data(self):
        """Loads skipped_words from the JSON file."""
        default_skipped = []
    ...
    def _save_data(self):
        """Saves the current skipped_words to the JSON file."""
        data_to_save = {
    ...
    def GetUnknownWordsViaDictionaryFromList(self, words_with_objects: List) -> list:
        ...
        for item_tuple in words_with_objects:
            text_content, textblock_obj = item_tuple
    ...
    def is_number(self, word: str) -> bool:
        """Return True if word is a numeric value (with optional currency symbols)."""
        return word.strip('\'').replace('$', '').replace('£', '').replace('¥', '').replace('¢', '').replace('.', '', 1).isdigit()